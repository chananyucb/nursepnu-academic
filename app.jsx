/* ===================================================================
   app.jsx — shell, routing, state, Supabase-aware CRUD
=================================================================== */
    ...
    <div style={{ display:'grid', placeItems:'center', height:'100vh', background:'#f3f6f5' }}>
      <div style={{ textAlign:'center' }}>
        ...
        <div style={{ fontFamily:'IBM Plex Sans Thai', color:'#9ca3af', fontSize:13, marginTop:6 }}>
          {window.db?.isConfigured() ? 'เชื่อมต่อ Supabase' : 'โหลดจากเครื่อง'}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ flex:1, fontSize:13.5, fontFamily:'IBM Plex Sans Thai', color:'#78350f' }}>
        <b>ยังไม่ได้เชื่อมต่อ Supabase</b> — ข้อมูลเก็บเฉพาะเครื่องนี้ คนอื่นจะไม่เห็น
      </div>
      <a href="setup-guide.html" target="_blank" style={{
...
function App() {
  ...
  useEffect(() => {
    Promise.all([window.db.fetchProjects(), window.db.fetchSpeakers()])
      ...
      .catch(err => {
        ...
        setLoading(false);
        toast('โหลดข้อมูลจาก Supabase ไม่ได้ ใช้ข้อมูลตัวอย่างแทน', 'danger');
      });
  }, []);
  ...
  const addProject = async (data) => {
    ...
    if (window.db.isConfigured()) {
      ...
      catch (e) { toast('บันทึก Supabase ล้มเหลว: ' + e.message, 'danger'); }
    }
  };
  ...
  const reallyDelete = async (id) => {
    ...
    if (window.db.isConfigured()) {
      ...
      catch (e) { toast('ลบ Supabase ล้มเหลว: ' + e.message, 'danger'); }
    }
  };
  ...
  const resetData = async () => {
    ...
    if (window.db.isConfigured()) {
      try { await window.db.seedAll(); toast('Seed ข้อมูลไปยัง Supabase แล้ว'); }
      catch (e) { toast('Seed Supabase ล้มเหลว: ' + e.message, 'danger'); }
    }
  };
    ...
    <div className={'app' + (navOpen ? ' nav-open' : '')}>
      ...
      <aside className="sidebar">
        ...
        <nav className="nav">
          ...
          <a href="setup-guide.html" target="_blank" className="nav-item" style={{ textDecoration:'none', color:'inherit' }}>
            <Icon name="eye" />คู่มือตั้งค่า Supabase
          </a>
        </nav>
        ...
        <div className="sidebar-foot">
          <div className="year-pill">
            ...
            <div style={{ flex:1 }}>
              <div className="lbl">
                {window.db.isConfigured() ? 'เชื่อมต่อ Supabase แล้ว' : 'โหมด localStorage'}
              </div>
